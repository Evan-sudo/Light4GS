...
class Deformation(nn.Module):
    ...
    def query_time(self, rays_pts_emb, scales_emb, rotations_emb, time_feature, time_emb):
        ...
        if self.no_grid:
            ...
            # input is (points, x,y,z) (points, timestamp) then interpolates and concat at hidden space
            grid_feature = self.grid(rays_pts_emb[:,:3], time_emb[:,:1])  # this function acts as the forward function of grid
            # breakpoint()
            if self.grid_pe > 1:
                grid_feature = poc_fre(grid_feature,self.grid_pe)
            hidden = torch.cat([grid_feature],-1) 
        
        hidden = self.feature_out(hidden)   # before deform, goes thru MLP
